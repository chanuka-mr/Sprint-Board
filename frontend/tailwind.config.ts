import type { Config } from "tailwindcss";

const config: Config = {
  darkMode: "class",
  content: [
    "./app/**/*.{js,ts,jsx,tsx,mdx}",
    "./components/**/*.{js,ts,jsx,tsx,mdx}",
    "./context/**/*.{js,ts,jsx,tsx,mdx}",
  ],
  theme: {
    extend: {
      colors: {
        board: {
          50: "rgb(var(--board-50) / <alpha-value>)",
          100: "rgb(var(--board-100) / <alpha-value>)",
          200: "rgb(var(--board-200) / <alpha-value>)",
          300: "rgb(var(--board-300) / <alpha-value>)",
          400: "rgb(var(--board-400) / <alpha-value>)",
          500: "rgb(var(--board-500) / <alpha-value>)",
          600: "rgb(var(--board-600) / <alpha-value>)",
          700: "rgb(var(--board-700) / <alpha-value>)",
          800: "rgb(var(--board-800) / <alpha-value>)",
          900: "rgb(var(--board-900) / <alpha-value>)",
        },
      },
    },
  },
  plugins: [],
};

export default config;